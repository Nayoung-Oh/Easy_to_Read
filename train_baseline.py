...
def simplify(model: torch.nn.Module, src_sentence: str):
    model.eval()
...
# # val loss 3.95
# min_val_loss = 10
# with open("training_baseline.csv", "w", newline="") as f:
#     writer = csv.writer(f)
...
#             print("save model")
#             torch.save(transformer.state_dict(), f"baseline_{epoch}_val_{val_loss:.3f}")
...
transformer.load_state_dict(torch.load("baseline_3_val_2.484"))
...
with open("./wikilarge/new_data_test.csv", encoding = 'utf-8') as f:
    with open("./wikilarge/baseline_report.txt", "w", encoding='utf-8') as res:
        reader = csv.reader(f)
        for l in reader:
            res.write(simplify(transformer, l[-2]) + '\n')
...
def total_output(src, dest, index, delta):
  ...
  new_dest = simplify(transformer, src)
