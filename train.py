...
def train_epoch(model, optimizer, save):
    model.train()
    losses = 0
    train_iter = WikiDataset("./wikismall/filtered_data.csv")
    ...
    if save:
        print("save model")
        torch.save(model.state_dict(), "trial")
...
def evaluate(model):
    model.eval()
    losses = 0
    val_iter =  WikiDataset("./wikismall/filtered_data_val.csv")
...
for epoch in range(1, NUM_EPOCHS+1):
    start_time = timer()
    train_loss = train_epoch(transformer, optimizer, save=(epoch==NUM_EPOCHS))
    end_time = timer()
