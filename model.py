...
class Seq2SeqTransformer(nn.Module):
    def __init__(self,
        ...
        self.cls_lin = nn.Linear(cls_siz, emb_size)
    ...
    def forward(self,
        ...
        fin_emb = torch.cat((cls_emb, src_emb), 0)
        tgt_emb = self.positional_encoding(self.tgt_tok_emb(trg))
        outs = self.transformer(fin_emb, tgt_emb, src_mask, tgt_mask, None, 
...
class Seq2SeqTransformerBaseline(nn.Module):
    def __init__(self,
                 num_encoder_layers: int,
                 num_decoder_layers: int,
                 emb_size: int,
                 ...
                 dim_feedforward: int = 512,
                 dropout: float = 0.1):
        super(Seq2SeqTransformerBaseline, self).__init__()
        self.transformer = Transformer(d_model=emb_size,
                                       nhead=nhead,
                                       num_encoder_layers=num_encoder_layers,
                                       num_decoder_layers=num_decoder_layers,
